...
[ $? -eq 0 ] && yum -y install kernel-devel rpm-build patch make gcc gcc-c++ flex bison file libxml2 libxml2-devel curl curl-devel libjpeg libjpeg-devel libtool libpng libpng-devel openssl* wget libaio* vim libmcrypt libmcrypt-devel mcrypt mhash || exit 39
